...
crea o aggiorna il file `.last`, committa e pusha il cambio."
    ...
    (let ((exit-code (call-process "make" nil "*rsync-upload*" t "rsync_upload")))
      (if (eq exit-code 0)
          (progn
            ...
            (call-process "git" nil nil t "add" last-file)
            (call-process "git" nil nil t "commit" "-m" "Update .last after rsync upload")
            (call-process "git" nil nil t "push")
            (message "Upload completato. File `.last` aggiornato, committato e pushato."))
