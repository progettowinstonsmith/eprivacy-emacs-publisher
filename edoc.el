  ...
  (let* ((src-path (expand-file-name edoc-current-edition-path))
    ...
    (let* ((year (completing-read "Anno destinazione: " years nil t nil nil (car years)))
      ...
      (let* ((session (completing-read "Sessione destinazione: " sessions nil t nil nil (car sessions)))
        ...
        (message "Copia completata a %s/%s" year session)))))
...
(defun edoc-save-current-edition ()
  "Salva l'edizione corrente come edizione di lavoro nel launcher."
  (interactive)
  (let* ((init-file (expand-file-name "~/epub-system/init-dashboard.el"))
         (current-path edoc-current-edition-path)
         (content (with-temp-buffer
                    (insert-file-contents init-file)
                    (goto-char (point-min))
                    (if (re-search-forward "^(setq edoc-current-edition-path \".*\")$" nil t)
                        (progn
                          (replace-match (format "(setq edoc-current-edition-path \"%s\")" current-path))
                          (buffer-string))
                      (user-error "Non trovo la riga edoc-current-edition-path in %s" init-file)))))
    (with-temp-file init-file
      (insert content))
    (message "Edizione di lavoro salvata: %s" current-path)))
